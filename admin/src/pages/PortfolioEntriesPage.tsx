...
export function PortfolioEntriesPage() {
  ...
  const rows = useMemo<Row[] | undefined>(() => {
    ...
    const names = new Map((cores.items ?? []).map((core) => [core.slug, core.name]));
    return list.items.map((item) => ({ ...item, coreName: names.get(item.coreSlug) ?? item.coreSlug }));
  ...
  const detailTemplate = (record: Row) => {
    ...
    return (
      <span className="admin-status">
        {record?.technologySlugs?.length || record?.technologies?.length || 0} tech · {record?.galleryMedia?.length ?? 0} images
  ...
  const actionsTemplate = (record: Row) => (
    <div style={{ display: "flex", gap: 6 }}>
      <ButtonComponent cssClass="e-small e-outline e-primary" onClick={() => navigate(`/portfolioEntries/${record.slug}`)}>
        Edit
      </ButtonComponent>
  ...
  return (
    <>
      <div className="admin-page-header">
        <div>
          <h1>Projects</h1>
          <p>Portfolio projects shown on both sites. Drag a column header to the bar above the grid to group.</p>
        </div>
        <ButtonComponent cssClass="e-primary e-outline" onClick={() => navigate("/portfolioEntries/new")}>
          Add project
        </ButtonComponent>
      </div>

      {list.isError ? <p className="admin-error">Could not load projects.</p> : null}
      ...
      <div className="admin-grid-wrap">
        <EntityGrid gridId="portfolioEntries" rows={rows} mode="local" onReorder={saveOrder}>
          {[
            ...
            <ColumnDirective key="coreName" field="coreName" headerText="Core" width={150} />,
            <ColumnDirective key="year" field="year" headerText="Year" width={90} />,
            <ColumnDirective key="detail" headerText="Contains" width={210} template={detailTemplate} allowSorting={false} allowFiltering={false} allowGrouping={false} />,
